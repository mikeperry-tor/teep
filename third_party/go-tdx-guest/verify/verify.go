...
type Options struct {
	verifiedValidity    time.Time
	hasVerifiedValidity bool
...
func TdxQuoteContext(ctx context.Context, quote any, options *Options) error {
	...
	options.verifiedValidity = time.Time{}
	options.hasVerifiedValidity = false
	if err := tdxQuote(ctx, quote, options); err != nil {
		return err
	}
	if options.GetCollateral && options.CheckRevocations {
		bound, err := options.collateralValidityBound()
		if err != nil {
			return err
		}
		options.verifiedValidity = bound
		options.hasVerifiedValidity = true
	}
	return nil
}
